			...
			if taxiId then
				...
				if amount ~= nil and amount > 0 then
					if not taxiState[user_id] then
						...
						vRPclient.notify(player,{"~b~Deja ai pornit aparatul!"})
					end
				else
					vRPclient.notify(player,{"~b~Tariful specificat este invalid!"})
				end
			else
				vRPclient.notify(player,{"~b~Nu te afli intr-un taxi!"})
			end
		end)
	end
end)
...
AddEventHandler('taxi:setFirstPasager', function (nplayer)
	...
	if taxiState[user_id] then
		...
		vRPclient.notify(nplayer,{"Te ai urcat in taxi!~n~Sofer:~b~ "..GetPlayerName(player).."~w~!~n~Tarif pe km: ~b~"..vRP.formatMoney({taxiState[user_id].fare})})
		...
		vRPCtaxi.showOnScreemHud(nplayer,{
			GetPlayerName(nplayer),
			taxiState[user_id].earnings,
			...
			if type(taxiState[user_id].passager) == 'table' then
				...
				if #(coords - taxiState[user_id].coords) > 100.0 then
					...
					if (nmoney - taxiState[user_id].earnings) < 1000 then
						vRPclient.notify(player,{"~b~Pasagerul aporape a ramas fara bani!"})
					end
				end
				vRPCtaxi.updateNameAndPaid(nplayer,{GetPlayerName(player),taxiState[user_id].earnings})
				vRPCtaxi.updateNameAndEarnings(nplayer,{GetPlayerName(nplayer),taxiState[user_id].earnings})
			end
		end
	end
end)
...
AddEventHandler('taxi:passagerLeft', function (driverId)
	...
	if taxiState[user_id] then
		...
		local payOut = taxiState[user_id].earnings
		if nplayer then
			if payOut > 0 then
				if vRP.tryFullPayment({user_id,payOut}) then
					vRP.giveMoney({nuser_id,payOut})
					vRPclient.notify(player,{"Ai primit ~b~"..vRP.formatMoney({payOut}).."$~w~ pentru cursa de ~b~Taxi~w~!"})
					vRPclient.notify(nplayer,{"Ai platit ~b~"..vRP.formatMoney({payOut}).."$~w~ pentru cursa de ~b~Taxi~w~!"})
				end
			end
			taxiState[user_id].earnings = 0
			taxiState[user_id].passager = nil
			taxiState[user_id].coords = nil
			...
			if nplayer then
				...
				if player then
					if vRP.tryFullPayment({user_id,taxAmount}) then
						vRPclient.notify(player,{"~b~Ai parasit masina in timpul cursei! Ai fost taxat cu ~b~"..vRP.formatMoney({taxAmount}).."$"})
					end
				end
			end
		end

...
Citizen.CreateThread(function()
	while true do
		...
		if weapon == GetHashKey("WEAPON_GADGETPISTOL") then	
			if IsPedShooting(ped) then
		...
		if weapon == GetHashKey("WEAPON_MG") then
			if IsPedShooting(ped) then
				ShakeGameplayCam('SMALL_EXPLOSION_SHAKE', 0.097)
			end
		end
		
		if weapon == GetHashKey("WEAPON_COMBATMG") then		
			if IsPedShooting(ped) then
				ShakeGameplayCam('SMALL_EXPLOSION_SHAKE', 0.089)
			end
		end
...
local recoils = {
	...
	[-1746263880] = 0.52, -- DOUBLE ACTION REVOLVER
